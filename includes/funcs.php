<?php
    ...
    function mysql_query_cache($sql, $linkIdentifier = false, $timeout = MEMCACHE_STATICDATA_TIMEOUT, $goSilent = true) {
        ...
        if (($cache = getCache(md5("mysqli_query" . $sql))) === false) {
			if (!$goSilent) dprintf("mysql_query_cache(): using MySQL data.", $sql);
            $cache = false;
            $r = ($linkIdentifier !== false) ? mysqli_query($linkIdentifier, $sql) : mysqli_query($sql);
            if (/*is_resource($r) && */(($rows = mysqli_num_rows($r)) !== 0)) {
